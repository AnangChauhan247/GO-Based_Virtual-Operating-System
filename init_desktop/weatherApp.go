...
func showWeatherApp(w fyne.Window){
	...
	// w:=a.NewWindow("Sharp Weather App")
	w.Resize(fyne.NewSize(500,500))
	//API import
	res, err:= http.Get("https://api.openweathermap.org/data/2.5/weather?q=delhi&APPID=44eb80c94ec05c69f625cfc68d806db0")
	if err!=nil{
		fmt.Println(err)
	}
	...
	label3:=canvas.NewText(fmt.Sprintf("Wind Speed %d",weather.Wind.Speed),color.White)
	label4:=canvas.NewText(fmt.Sprintf("Tempurature %.2f",weather.Main.Temp),color.White)
	...
	w.SetContent(container.NewBorder(panelContent,nil,nil,nil,content),)
